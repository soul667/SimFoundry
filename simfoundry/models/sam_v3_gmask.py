...
class SAM3(torch.nn.Module):
    ...
    def _image_inference_context(self):
        return torch.autocast(device_type="cuda", dtype=torch.bfloat16) if str(self.device).startswith("cuda") else nullcontext()
    ...
    def predict_segmentation_with_point(self, pil_img, point_coords, point_labels=None, multimask_output=False):
        """
        Predict segmentation using point prompts instead of text prompts.
        ...
        # Normalize point_coords to list of tuples
        ...
        # Default labels: all foreground
        if point_labels is None:
            point_labels = [1] * len(point_coords)
        
        # Convert to numpy arrays for SAM
        points_np = np.array(point_coords, dtype=np.float32)  # Shape: (N, 2)
        ...
        with self._image_inference_context():
            ...
            # Use the underlying SAM model for point prompt
            # The Sam3Processor wraps the raw model, we need to access it for point prompts
            raw_model = self.model.model
            
            if raw_model.inst_interactive_predictor is None:
                raise RuntimeError(
                    "Point-prompt inference requires SAM3 interactive predictor. "
                    "Rebuild image model with enable_inst_interactivity=True."
                )

            # Predict with point prompt through SAM3 interactive path
            # API expects points in (x, y) pixel coordinates.
            masks, scores, logits = raw_model.predict_inst(
        ...
        # Convert masks to expected format (N, 1, H, W)
        if len(masks.shape) == 3:
            # masks shape is (N, H, W), add channel dimension
    ...
    def prune_redundant_masks(
        ...
        for i, mask in enumerate(masks):
            # skimage expects bool/integer masks; keep behavior robust to float upcasting.
