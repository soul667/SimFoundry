...
def axisangle2mat(vec):
    """
    Converts scaled axis-angle to matrix.
...
def umeyama_alignment(src, dst, with_scale=True):
    """Least-squares similarity/rigid alignment mapping @src points onto @dst (Umeyama 1991).

    Solves for (s, rot, t) minimizing || dst - (s @ rot @ src + t) ||^2.

    Args:
        src (np.array): (N, 3) source points.
        dst (np.array): (N, 3) target points (same shape as @src).
        with_scale (bool): If True, estimate a uniform scale (Sim(3)); if False, solve a
            rigid SE(3) transform with s = 1.

    Returns:
        2-tuple:
            - (np.array): 4x4 homogeneous transform T such that dst ~= (T @ [src, 1]^T)[:3].
              When @with_scale is True the rotation block already includes the scale
              (T[:3, :3] = s * rot).
            - (float): estimated scale s (1.0 when @with_scale is False).
    """
    src = np.asarray(src, dtype=np.float64)
    dst = np.asarray(dst, dtype=np.float64)
    assert src.shape == dst.shape and src.ndim == 2 and src.shape[1] == 3, "src and dst must be (N, 3)"
    n = src.shape[0]
...
def camera_centers_from_world2cam(world2cam):
    """Camera center(s) in world coordinates from world2cam extrinsic(s): c = -rot^T @ t.

    Args:
        world2cam (np.array): A single (3, 4) or (4, 4) world2cam extrinsic, or a batch of
            them with shape (N, 3, 4) or (N, 4, 4).

    Returns:
        np.array: (3,) camera center for a single extrinsic, or (N, 3) for a batch.
    """
    ext = np.asarray(world2cam, dtype=np.float64)
    if ext.ndim == 2:
        rot = ext[:3, :3]
        t = ext[:3, 3]
        return -rot.T @ t
    if ext.ndim == 3:
        rots = ext[:, :3, :3]
        ts = ext[:, :3, 3]
        return -np.einsum("nji,nj->ni", rots, ts)
    raise ValueError(f"world2cam must be 2D or 3D, got shape {ext.shape}")
