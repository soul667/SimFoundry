...
"""Stage 8b writes articulation results; stage 10 reads them back.

Regression cover for a writer/reader mismatch: stage 8b used the raw scene name and a
non-lowercased object name, while stage 10 lowercased both. A scene named ``Laptop``
...
STAGE_8B = REPO_ROOT / "scripts/pipeline/A_reconstruction/stages/8b_articulate_objects.py"
STAGE_10 = REPO_ROOT / "scripts/pipeline/A_reconstruction/stages/10_make_objects_sim_ready.py"


def _load_resolver():
    """Pull the resolver out of stage 10 without importing its heavy dependencies."""
    src = STAGE_10.read_text(encoding="utf-8")
...
def test_writer_and_reader_agree_on_a_capitalized_scene(tmp_path):
    ...
    out_dir = tmp_path / "s8b_articulate_objects"

    # Writer side, mirroring 8b: both components sanitized.
...
def test_both_stages_use_the_shared_sanitizer():
    ...
    assert 'replace(" ", "_").replace("/", "_")\n' not in STAGE_8B.read_text(encoding="utf-8")
