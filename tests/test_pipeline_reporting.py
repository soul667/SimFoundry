...
def test_scene_manifest_records_stable_stage_invariants(tmp_path):
    ...
    (scene_dir / "s10_sim").mkdir(parents=True)
    ...
    (scene_dir / "s10_sim" / "scene_objects_info.json").write_text(json.dumps({"1": {}}), encoding="utf-8")

    manifest = build_scene_manifest(scene_dir)

    assert manifest["invariants"]["s1_subsampled_frames"] == 1
    assert manifest["invariants"]["s10_scene_objects"] == 1
