...
def load_stage12_module(monkeypatch, repo_root: Path, dataset_root: Path):
    ...
    module_path = repo_root / "scripts" / "pipeline" / "A_reconstruction" / "stages" / "12_import_usd.py"
    name = "stage12_import_usd_for_tests"
...
def test_import_usd_asset_uses_configured_dataset_and_fails_fast(monkeypatch, tmp_path):
    ...
    stage12 = load_stage12_module(monkeypatch, repo_root, tmp_path / "datasets")
    ...
    def fake_run(cmd, check):
        ...
        usd_path = Path(stage12.expected_usd_path(dataset_name, category, model))
        usd_path.parent.mkdir(parents=True)
        usd_path.write_text("#usda", encoding="utf-8")

    monkeypatch.setattr(stage12.subprocess, "run", fake_run)

    usd_path = stage12.import_usd_asset(
...
def test_import_usd_asset_raises_when_importer_creates_no_usd(monkeypatch, tmp_path):
    ...
    urdf_path = tmp_path / "cup.urdf"
    urdf_path.write_text("<robot />", encoding="utf-8")
    monkeypatch.setattr(stage12.subprocess, "run", lambda cmd, check: None)

    with pytest.raises(FileNotFoundError, match="USD importer did not create expected asset"):
        stage12.import_usd_asset(
            dataset_name="real2sim-assets",
            urdf_path=str(urdf_path),
            category="white_cup",
            model="abcdef",
